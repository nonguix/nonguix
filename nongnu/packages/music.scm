...
(define %reaper-source-x86_64-linux
  (package
    ...
    (source
     (origin
       (method url-fetch)
       (uri (string-append
             "https://www.reaper.fm/files/" (version-major version) ".x/reaper"
             (string-replace-substring version "." "") "_linux_x86_64.tar.xz"))
       (sha256
        (base32 "1w8dsrdca72c2cznww869f1j206d6743hjf3afp4p02lgr1rckf9"))))))

(define %reaper-source-i686-linux
  (package
    (inherit %binary-source)
    (version "7.77")
    (source
     (origin
       ...
       (uri (string-append
             "https://www.reaper.fm/files/" (version-major version) ".x/reaper"
             (string-replace-substring version "." "") "_linux_i686.tar.xz"))
       (sha256
        (base32 "0iz27k59plcz1rfgs2fdyyrd7l5g2hsk8m001vnnkysxgrmy6xz1"))))))

(define %reaper-source-aarch64-linux
  (package
    (inherit %binary-source)
    (version "7.77")
    (source
     (origin
       ...
       (uri (string-append
             "https://www.reaper.fm/files/" (version-major version) ".x/reaper"
             (string-replace-substring version "." "") "_linux_aarch64.tar.xz"))
       (sha256
        (base32 "1kfn9ybk8sfhrg4r9nqy06sawqx45gjc4f0b3arzbrpvl6qwywgz"))))))

(define %reaper-source-armhf-linux
  (package
    (inherit %binary-source)
    (version "7.77")
    (source
     (origin
       ...
       (uri (string-append
             "https://www.reaper.fm/files/" (version-major version) ".x/reaper"
             (string-replace-substring version "." "") "_linux_armv7l.tar.xz"))
       (sha256
        (base32 "0wzciamkz2nqqsi7xrckfdwjczhf0faidn4fyqv6s2k6hqnyb80d"))))))

(define-public reaper
  (binary-package-from-sources
   `(("x86_64-linux"  . ,%reaper-source-x86_64-linux)
     ("i686-linux"    . ,%reaper-source-i686-linux)
     ("aarch64-linux" . ,%reaper-source-aarch64-linux)
     ("armhf-linux"   . ,%reaper-source-armhf-linux))
   (package
     (name "reaper")
     (version #f)
     (source #f)
